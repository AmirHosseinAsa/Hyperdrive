...
public class LevelSelection : MonoBehaviour
{
    ...
    private IEnumerator SelectLevel()
    {
        ...
        for (int i = 5; i < Menu.ins.totalNumLevels; i++)
        {
            Menu.ins.levelsLayout.gameObject.transform.GetChild(i).gameObject.SetActive(!(Menu.ins.gameType != Menu.GameTypes.play));
        }

        for (int i = 0; i < (Menu.ins.gameType == Menu.GameTypes.play ? Menu.ins.totalNumLevels : 5); i++)
        {
