...
public class Player : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.Joystick1Button1)) && (controlType == Player.ControlTypes.human) && (!GameManager.instance.UIController.racePaused) && (totalRaceTime > 3f) && (!recentReset))
        { StartCoroutine(ResetCarPosition()); recentReset = true; StartCoroutine(UpdateRecentReset()); }
        else if ((Input.GetKeyDown(KeyCode.Tab) || Input.GetKeyDown(KeyCode.Joystick2Button1)) && (controlType == Player.ControlTypes.humanB) && (!GameManager.instance.UIController.racePaused) && (totalRaceTime > 3f) && (!recentReset))
        { StartCoroutine(ResetCarPosition()); recentReset = true; StartCoroutine(UpdateRecentReset()); }


        if ((Input.GetKeyDown(KeyCode.RightControl) || Input.GetKeyDown(KeyCode.Joystick1Button3)) && (controlType == Player.ControlTypes.human) && (!GameManager.instance.UIController.racePaused) && (totalRaceTime > 3f) && (!recentReset))
        {
            CameraFollow.ins.ChangeCameraViewWrapper();
        }

        if ((Input.GetKeyDown(KeyCode.LeftControl) || Input.GetKeyDown(KeyCode.Joystick2Button3)) && (controlType == Player.ControlTypes.humanB) && (!GameManager.instance.UIController.racePaused) && (totalRaceTime > 3f) && (!recentReset))
        {
            CameraFollowB.ins.ChangeCameraViewWrapperB();
        }
    }
